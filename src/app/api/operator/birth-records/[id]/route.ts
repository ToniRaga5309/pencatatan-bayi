...
const updateBirthRecordSchema = z.object({
  ...
  namaBayi: z.string().min(3).max(100).optional(),
  tanggalLahir: z.string().optional(),
  tempatLahir: z.string().min(2).max(100).optional(),
  jenisKelamin: z.enum(["LAKI_LAKI", "PEREMPUAN"]).optional(),
  beratBadan: z.number().min(0.5).max(10).optional().nullable(),
  panjangBadan: z.number().min(20).max(70).optional().nullable()
})
...
// PUT: Update data kelahiran (hanya jika status PENDING)
  ...
  try {
    ...
    if (data.nikIbu) {
      ...
      if (!nikValidation.valid) {
        return NextResponse.json({ error: nikValidation.message }, { status: 400 })
      }
    }
    ...
    const updatedRecord = await db.birthRecord.update({
      ...
      data: {
        ...
        beratBadan: data.beratBadan !== undefined ? data.beratBadan : undefined,
        panjangBadan: data.panjangBadan !== undefined ? data.panjangBadan : undefined
      }
    })
...
// DELETE: Soft delete data (hanya jika status PENDING)
export async function DELETE(
  ...
  try {
    ...
    if (existingRecord.status !== "PENDING") {
      return NextResponse.json(
        { error: "Hanya data dengan status Menunggu yang dapat dihapus" },
        { status: 400 }
      )
    }
